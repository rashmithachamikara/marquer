...
float pidOutput;
float setpoint = 0; // Desired gyroZ value (target angle)
...
void loop() {
  ...
  // Serial.print(encoder2Count / ENCODER_RESOLUTION);
  lastTime = currentTime;
  ...
  if (imu.Read()) {
    ...
    gyroZ = roundf(((imu.gyro_z_radps()+0.02) * 1000) / 100);
    //gyroZ = imu.gyro_z_radps();//+0.02;
    gyroZTotal += gyroZ;
    ...
    // Compute PID output
    if(speedA >= 10 and speedB >= 10){
      // Adjust motor speeds based on PID output
      speedA = constrain(speedA - pidOutput, 80, 250);
      speedB = constrain(speedB + pidOutput, 80, 250);
    }
    

    lastError = error;

    Serial.print(gyroZ,7);
    Serial.print("\t");
    Serial.print(gyroZTotal,7);
    Serial.print("\t");
