...
void correctTurnGap(){
  int slowingDistance=5; //Slow down from last 20cm
...
//Correct turn gap afterturning
void correctAfterTurnGap(){
  int slowingDistance=2; //Slow down from last 20cm

  remaigningDistance = targetDistance-wheelDistance;

  if (wheelDistance < afterTurnGapDistance){
    if (speedA < minPidSpeed){
      speedA = movingBaseSpeed; //Make speed a bit more than pid baseline
    }
    if (speedB < minPidSpeed){
    ...
    delay(100);
    //Print debug stuff then set flag
    WebPrintln("After Turn Gap compensation made. Moved a distance of "+String(wheelDistance)+"cm.");
    correctingAfterTurnGap = false;
