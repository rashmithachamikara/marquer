...
Ticker displayLogoTicker;
Ticker displayReinitTicker;
//======================================
...
unsigned long pidInterval = 50;
unsigned long lastTime = 0;
unsigned long lastPidUpdateTime = 0;
int minPidSpeed = 60; //60;
int maxPidSpeed = 140; //180;
...
int turnBaseSpeed = 40; //Speed for turning. 100 for 7v+ (2x 3.7v). 60 for 12v+ (3x 3.7v). 30 with PCB as low wire resistance
double turnError;
...
void setup() {
  ...
  setWebMessageCallback([](String message) {
    ...
    } else if(message.startsWith("Callib")){
      handleCallib(message.substring(7)); //Remote commands
...
void loop() {
  ...
  //=========== PID ===========
  //Do PID per 50ms
  if (currentTime - lastPidUpdateTime >= pidInterval){
    moveStraightPid();
  }
  //===========================
  ...
  if (currentTime - lastSerialUpdateTime >= 300) {
    ...
    WebPrintln(
                ...
                ", sR:"+String(speedA)+
                ", Te:"+String(turnError)+
                ", Ti:"+String(turnIntegral)+
                ", Td:"+String(turnDerivative)+
                ", tPid:"+String(turnPidOutput)+
                ", yaw:"+String(yaw)+
...
void distanceMove(){
  ...
  if (wheelDistance < targetDistance){
    ...
    if (executingCommandList) {
      ...
      wheelDistance = 0;
      //Set new target
      targetDistance = turnGapDistance;
      ...
      //Correct turn error
      yawCorrection = yaw * -1;
      WebPrintln("Correcting before move gap stop rotation of " + String(yaw) + ". Yaw Correction:" + String(yawCorrection));
      //handleInput("T"+String(yawCorrection));
      String argument = "T"+String(0); //Yaw correction will now be added later
...
void correctTurnGap(){
  ...
  if (wheelDistance < targetDistance){
    ...
    //Correct turn error
    yawCorrection += yaw*-1;
    WebPrintln("Correcting after move gap stop rotation of " + String(yaw) + ". Yaw Correction: " + String(yawCorrection));
    //handleInput("T"+String(yawCorrection));

    //Turning system has been commented out. No need to execute next command as turning system does it after correcting yaw
    if (executingCommandList) {
      nextCommand();
    }
  }
