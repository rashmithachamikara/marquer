...
void setupMarquerWeb() {
  ...
  // Route to serve the controlAppPid.html file when /marquer is accessed
  server.on("/marquerCallib", HTTP_GET, [](AsyncWebServerRequest *request){
    Serial.println("Serving /marquerCallib");
    if (LittleFS.exists("/controlAppCallib.html")) {
      request->send(LittleFS, "/controlAppCallib.html", "text/html");
      Serial.println("File served successfully");
    } else {
      request->send(404, "text/plain", "File not found");
      Serial.println("File not found");
